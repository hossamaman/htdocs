...
| 
| 
| -> AUTHOR / HASSAN AZZI & https://github.com/neitanod/forceutf8
| -> DATE / 2018-02-18
...
class Encoding 
{
    ...
    public static function fixUTF8($text, $option = self::WITHOUT_ICONV)
    {
        ...
        while($last <> $text){
            $last = $text;
            $text = self::utf8(static::utf8_decode($text, $option));
        }
        $text = self::utf8(static::utf8_decode($text, $option));
        return $text;
    }
    ...
    protected static function utf8_decode($text, $option)
    {
        if ($option == self::WITHOUT_ICONV || !function_exists('iconv')) {
             $o = utf8_decode(
                 str_replace(array_keys(self::$utf8ToWin1252), array_values(self::$utf8ToWin1252), self::utf8($text))
             );
        } else {
